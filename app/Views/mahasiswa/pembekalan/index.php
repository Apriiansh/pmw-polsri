        ...
        <div class="flex items-start gap-3">
            ...
            <div>
                ...
                <?php if (!empty($announcement->training_details)): ?>
                    <p class="text-[12px] text-slate-500 mt-2"><?= nl2br(esc($announcement->training_details)) ?></p>
                <?php endif; ?>
            </div>
        </div>
    </div>
    ...
    return {
        ...
        deletePhoto(photoId) {
            ...
            .then(d => {
                ...
                window.dispatchEvent(new CustomEvent('toast-notify', { detail: { message: d.message || 'Gagal menghapus foto', type: 'error' } }));
            })
            .catch(() => {
                window.dispatchEvent(new CustomEvent('toast-notify', { detail: { message: 'Terjadi kesalahan server', type: 'error' } }));
            });
        }
    }
}
