...
<div class="space-y-8 max-w-6xl mx-auto" x-data="implementasiMahasiswa()">
    ...
    <div class="flex flex-col sm:flex-row sm:items-center sm:justify-between gap-4 animate-stagger">
        <div>
            <h2 class="section-title text-xl sm:text-2xl">
                Implementasi List <span class="text-gradient">Perjanjian</span>
            </h2>
        ...
        <?php if ($selection && $selection->admin_status === 'revision' && $canEdit): ?>
            <button @click="resetAll()"
                class="btn-outline btn-sm bg-rose-50 text-rose-700 border-rose-200 hover:bg-rose-500 hover:text-white group">
    ...
    <div class="grid md:grid-cols-4 gap-5 animate-stagger delay-100">
        ...
        <div class="card-premium p-5 border-l-4 <?= $isPhaseOpen ? 'border-l-emerald-500' : 'border-l-rose-500' ?>"
            ...
            <span
                class="inline-flex items-center gap-2 px-3 py-1 rounded-full text-[10px] font-black mt-3 <?= $isPhaseOpen ? "bg-emerald-50 text-emerald-700" : "bg-rose-50 text-rose-700" ?>">
    ...
    <div class="grid md:grid-cols-3 gap-6 animate-stagger delay-150">
        <div class="card-premium p-5 flex items-center justify-between group overflow-hidden"
            @mousemove="handleMouseMove">
            <div class="relative z-10">
                ...
                ?>
                <p class="text-2xl font-bold text-sky-600 mt-1"><?= $totalQty ?> <span
                        class="text-xs text-slate-400 font-normal">item belanja</span></p>
            </div>
            <div
                class="w-14 h-14 rounded-2xl bg-sky-50 flex items-center justify-center group-hover:bg-sky-100 transition-colors duration-500">
        ...
        <div class="card-premium p-5 flex items-center justify-between group overflow-hidden"
            ...
            <div class="relative z-10">
                ...
                <p class="text-2xl font-bold text-violet-600 mt-1">Rp
                    <?= number_format($proposal['total_rab'] ?? 0, 0, ',', '.') ?></p>
            </div>
            <div
                class="w-14 h-14 rounded-2xl bg-violet-50 flex items-center justify-center group-hover:bg-violet-100 transition-colors duration-500">
        ...
        <div class="card-premium p-5 flex items-center justify-between group overflow-hidden"
            ...
            <div class="relative z-10">
                ...
                <p class="text-2xl font-bold text-emerald-600 mt-1">Rp <?= number_format($totalPrice, 0, ',', '.') ?>
                </p>
            </div>
            <div
                class="w-14 h-14 rounded-2xl bg-emerald-50 flex items-center justify-center group-hover:bg-emerald-100 transition-colors duration-500">
    ...
    <?php if ($isPhaseOpen): ?>
        <div
            class="sticky top-4 z-20 bg-white/80 backdrop-blur-xl shadow-2xl shadow-sky-500/10 border border-white/40 rounded-3xl p-4 animate-stagger delay-150 flex items-center justify-between gap-4 flex-wrap ring-1 ring-slate-900/5">

            <!-- Left: Status info -->
            <div class="flex items-center gap-4 min-w-0">
                <?php if ($selectionStatus === 'approved'): ?>
                    <div
                        class="w-12 h-12 rounded-2xl bg-emerald-100 flex items-center justify-center shrink-0 border border-emerald-200">
                        <i class="fas fa-check-double text-emerald-600 text-lg"></i>
                    </div>
                    <div class="min-w-0">
                        <p class="text-[10px] font-black uppercase tracking-widest text-slate-400 leading-none">Status
                            Implementasi</p>
                        <p class="text-sm font-black text-emerald-700 mt-1">Selesai & Disetujui ✓</p>
                    </div>
                <?php elseif ($isSubmitted && $dosenStatus === 'pending'): ?>
                    <div
                        class="w-12 h-12 rounded-2xl bg-amber-100 flex items-center justify-center shrink-0 border border-amber-200">
                    ...
                    <div class="min-w-0">
                        ...
                        <p class="text-[9px] text-amber-500 font-bold bg-amber-50 px-2 py-0.5 rounded-full inline-block mt-0.5">
                            <i
                                class="fas fa-clock mr-1"></i><?= $selection->student_submitted_at ? date('d M Y H:i', strtotime($selection->student_submitted_at)) : '-' ?>
                        </p>
                    </div>
                <?php elseif ($isSubmitted && $dosenStatus === 'approved' && $selectionStatus === 'pending'): ?>
                    <div
                        class="w-12 h-12 rounded-2xl bg-sky-100 flex items-center justify-center shrink-0 border border-sky-200">
                ...
                <?php elseif ($canEdit): ?>
                    <div
                        class="w-12 h-12 rounded-2xl bg-slate-100 flex items-center justify-center shrink-0 border border-slate-200">
            ...
            <div class="flex items-center gap-3 shrink-0">
                <?php if ($canEdit): ?>
                    ...
                    <button @click="submitImplementation()" :disabled="isSubmitting"
        ...
        <div class="card-premium overflow-hidden animate-stagger delay-200" @mousemove="handleMouseMove">
            <div class="px-5 sm:px-7 py-4 border-b border-sky-50 bg-white/60 flex items-center justify-between">
                <div>
                    ...
                    <p class="text-[10px] text-slate-400 font-semibold mt-0.5 uppercase tracking-tighter">Input rincian
                        komponen implementasi sesuai RAB</p>
                </div>
            </div>
                ...
                <form @submit.prevent="saveItem()" class="grid md:grid-cols-12 gap-6">
                    <div class="md:col-span-6 form-field">
                        ...
                        <div class="input-group">
                            ...
                            <input type="text" x-model="newItem.item_title"
                                placeholder="Contoh: Oven Listrik, Tepung, NIB, dll" required>
                        </div>
                    </div>
                    <div class="md:col-span-6 form-field">
                    ...
                    <div class="md:col-span-6 form-field">
                        ...
                        <div class="input-group p-0!">
                            ...
                            <textarea x-model="newItem.item_description" rows="3" class="px-0! py-3!"
                                placeholder="Jelaskan secara ringkas untuk apa komponen ini digunakan dalam usaha Anda..."></textarea>
                        </div>
                    </div>
    ...
    <div class="card-premium overflow-hidden animate-stagger delay-250" @mousemove="handleMouseMove">
        <div class="px-5 sm:px-7 py-4 border-b border-sky-50 bg-white/60 flex items-center justify-between">
            ...
            <span
                class="text-[10px] font-black bg-sky-100 text-sky-700 px-3 py-1 rounded-full uppercase"><?= count($items) ?>
                Komponen</span>
        </div>
        <div class="p-5 sm:p-7 space-y-6">
            <?php foreach ($items as $item): ?>
                <div
                    class="group/item relative p-6 rounded-2xl border border-slate-200 bg-white hover:border-sky-300 hover:shadow-lg hover:shadow-sky-500/5 transition-all duration-300">
                    <div class="flex flex-col sm:flex-row sm:items-start justify-between gap-4">
                        <div class="flex-1">
                            <div class="flex items-center gap-3">
                                <div
                                    class="w-10 h-10 rounded-xl bg-slate-50 flex items-center justify-center shrink-0 border border-slate-100 group-hover/item:bg-sky-50 group-hover/item:border-sky-200 transition-colors">
                                    <i
                                        class="fas fa-box text-slate-400 group-hover/item:text-sky-500 transition-colors"></i>
                                </div>
                                <div>
                                    <?php
                                    ...
                                    if ($itemCategory):
                                        ...
                                        ?>
                                        <span
                                            class="text-[9px] font-black uppercase tracking-wider px-2 py-0.5 rounded-md <?= $catClass ?>"><?= $catLabel ?></span>
                                    <?php endif; ?>
                                    <h4 class="font-display font-bold text-base text-(--text-heading) mt-1">
                                        <?= esc($item->item_title) ?></h4>
                                    <div class="flex items-center gap-2 mt-0.5">
                                        ...
                                        <span
                                            class="text-[10px] font-black uppercase text-slate-500 bg-slate-100 px-2 py-0.5 rounded-md"><?= $item->qty ?>
                                            pcs</span>
                                        <span class="w-1 h-1 rounded-full bg-slate-200"></span>
                                        <span class="text-xs font-bold text-emerald-600">Total: Rp
                            ...
                            if ($itemDesc):
                                ?>
                                <div
                                    class="mt-4 p-3 rounded-xl bg-slate-50/50 border border-slate-100 text-[13px] text-slate-600 leading-relaxed">
                    ...
                    <div class="mt-6 pt-6 border-t border-slate-100">
                        <div class="flex items-center justify-between mb-4">
                            <p class="text-[10px] font-black text-slate-400 uppercase tracking-widest">
                                <i class="fas fa-camera-retro mr-1.5 text-sky-400"></i>Galeri Komponen
                                (<?= count((is_object($item) ? $item->photos : ($item['photos'] ?? [])) ?? []) ?>)
                            </p>
                        </div>
                        ...
                        <div class="grid grid-cols-2 sm:grid-cols-3 md:grid-cols-4 lg:grid-cols-5 gap-4">
                            ...
                            foreach ($itemPhotos as $photo):
                                ?>
                                <div
                                    class="group/img relative aspect-square rounded-2xl overflow-hidden border border-slate-100 bg-slate-50">
                                    ...
                                    <div
                                        class="absolute inset-0 bg-gradient-to-t from-black/80 via-black/20 to-transparent opacity-0 group-hover/img:opacity-100 transition-all duration-300 flex flex-col justify-end p-3">
                                        <div class="flex items-center justify-center gap-2 mb-2">
                                            <button
                                                @click="openLightbox('<?= base_url('mahasiswa/implementasi/photo/' . (is_object($photo) ? $photo->id : $photo['id'])) ?>', '<?= esc(is_object($photo) ? $photo->photo_title : $photo['photo_title']) ?>')"
                                        ...
                                        <p
                                            class="text-[9px] font-bold text-white truncate text-center uppercase tracking-tighter">
                            ...
                            <?php if ($canEdit && count($itemPhotos) < 5): ?>
                                ...
                                <div class="col-span-1">
                                    <div x-show="currentUploadItemId !== <?= is_object($item) ? $item->id : $item['id'] ?>"
                                        ...
                                        <input type="file" x-ref="photoInput<?= is_object($item) ? $item->id : $item['id'] ?>"
                                            class="hidden" accept=".jpg,.jpeg,.png"
                                            @change="handlePhotoSelected(<?= is_object($item) ? $item->id : $item['id'] ?>, $event)">
                                        <div
                                            class="w-10 h-10 rounded-full bg-white flex items-center justify-center shadow-sm group-hover/upload:scale-110 group-hover/upload:text-sky-500 transition-all mb-2">
                                            <i class="fas fa-cloud-arrow-up text-slate-400 group-hover/upload:text-sky-500"></i>
                                        </div>
                                        <p
                                            class="text-[10px] font-black text-slate-400 uppercase tracking-tighter group-hover/upload:text-sky-600 transition-colors">
                                            Tambah Foto</p>
                                    </div>
                                </div>
                            <?php endif; ?>
                        ...
                        <?php if ($canEdit): ?>
                            ...
                            <div x-show="currentUploadItemId === <?= is_object($item) ? $item->id : $item['id'] ?> && photoUploadForm.preview"
                                ...
                                <div class="flex flex-col md:flex-row items-center gap-6">
                                    <div
                                        class="relative w-32 h-32 shrink-0 group/preview rounded-2xl overflow-hidden shadow-lg border-2 border-white ring-4 ring-sky-100">
                                        <img :src="photoUploadForm.preview" class="w-full h-full object-cover">
                                        <button @click="cancelPhotoUpload()"
                                            class="absolute top-1 right-1 w-6 h-6 rounded-lg bg-black/50 text-white text-[10px] flex items-center justify-center opacity-0 group-hover/preview:opacity-100 transition-opacity">
                                            <i class="fas fa-times"></i>
                                        </button>
                                    </div>
                                    <div class="flex-1 w-full space-y-4">
                                        <div class="form-field">
                                            <label class="form-label text-sky-700">Beri Nama Foto Ini <span
                                                    class="required text-sky-400">*</span></label>
                                            <div class="input-group bg-white!">
                                                <div class="input-icon"><i class="fas fa-signature text-sky-400"></i></div>
                                                <input type="text" x-model="photoUploadForm.photo_title"
                                                    placeholder="Misal: Foto Detail Mesin, Kwitansi, dll" class="text-sm!"
                                                    required>
                                            </div>
                                        </div>
                                        <div class="flex flex-wrap gap-2">
                                            <button @click="submitItemPhoto(<?= is_object($item) ? $item->id : $item['id'] ?>)"
                                                :disabled="isLoadingPhoto || !photoUploadForm.photo_title"
                                                class="btn-primary btn-sm flex-1 sm:flex-none">
                                                <i class="fas "
                                                    :class="isLoadingPhoto ? 'fa-spin fa-spinner' : 'fa-cloud-arrow-up mr-2'"></i>
                                                <span x-text="isLoadingPhoto ? 'Mengupload...' : 'Mulai Upload'"></span>
                                            </button>
                                            <button @click="cancelPhotoUpload()"
                                                class="btn-outline btn-sm bg-white! flex-1 sm:flex-none">Batalkan</button>
                                        </div>
                                    </div>
                                </div>
                            </div>
            ...
            <?php if (empty($items)): ?>
                <div
                    class="flex flex-col items-center justify-center py-16 text-slate-300 bg-slate-50/50 rounded-3xl border-2 border-dashed border-slate-100">
                    ...
                    <p class="text-xs font-medium text-slate-400 mt-1 uppercase tracking-widest">Gunakan form di atas untuk
                        menambah rincian komponen implementasi</p>
                </div>
            <?php endif; ?>
        </div>
    </div>
    ...
    <div class="card-premium overflow-hidden animate-stagger delay-300" @mousemove="handleMouseMove">
        <div class="px-5 sm:px-7 py-4 border-b border-sky-50 bg-white/60 flex items-center justify-between">
            <div>
                ...
                <p class="text-[10px] text-slate-400 font-semibold mt-0.5 uppercase tracking-tighter">Upload semua nota,
                    invoice, atau slip pembayaran dalam satu tempat</p>
            </div>
            <span
                class="text-[10px] font-black bg-emerald-100 text-emerald-700 px-3 py-1 rounded-full uppercase tracking-widest"><?= count($payments) ?>
                Nota</span>
        </div>
        <div class="p-5 sm:p-7">
            <!-- Payment List Grid -->
            <div class="grid grid-cols-2 sm:grid-cols-3 md:grid-cols-4 lg:grid-cols-6 gap-4 mb-8">
                <?php foreach ($payments as $payment): ?>
                    <div
                        class="group relative aspect-[3/4] rounded-2xl overflow-hidden border border-slate-200 bg-slate-50 hover:shadow-xl hover:shadow-emerald-500/10 transition-all duration-300">
                        <img src="<?= base_url('mahasiswa/implementasi/payment/' . $payment->id) ?>"
                            alt="<?= esc($payment->payment_title) ?>" class="w-full h-full object-cover">

                        <div
                            class="absolute inset-0 bg-gradient-to-t from-emerald-950/90 via-emerald-900/20 to-transparent opacity-0 group-hover:opacity-100 transition-all duration-300 flex flex-col justify-end p-3">
                            <div class="flex items-center justify-center gap-2 mb-2">
                                <button
                                    @click="openLightbox('<?= base_url('mahasiswa/implementasi/payment/' . $payment->id) ?>', '<?= esc($payment->payment_title) ?>')"
                                    class="w-8 h-8 rounded-full bg-white/20 backdrop-blur-md text-white flex items-center justify-center hover:bg-white/40 transition-colors">
                                    <i class="fas fa-eye text-xs"></i>
                                </button>
                                <?php if ($canEdit): ?>
                                    <button
                                        @click="openEditPayment(<?= $payment->id ?>, '<?= esc($payment->payment_title) ?>', '<?= esc($payment->link_pembelian ?? '') ?>')"
                                        class="w-8 h-8 rounded-full bg-white/20 backdrop-blur-md text-white flex items-center justify-center hover:bg-white/40 transition-colors">
                            ...
                            <p
                                class="text-[10px] font-bold text-white text-center line-clamp-2 uppercase leading-tight tracking-tighter">
            ...
            <?php if ($canEdit): ?>
                ...
                <div
                    class="p-6 rounded-3xl bg-slate-50/50 border-2 border-dashed border-slate-200 hover:border-emerald-300 hover:bg-emerald-50/10 transition-all">
                    <form @submit.prevent="uploadPayment()" class="space-y-6">
                        <div class="grid md:grid-cols-12 gap-6">
                            <div class="md:col-span-4 form-field">
                                ...
                                <div class="input-group bg-white!">
                                    ...
                                    <input type="text" x-model="newPayment.payment_title"
                                        placeholder="Contoh: Nota Toko ATK Sejahtera" required>
                                </div>
                            </div>
                            <div class="md:col-span-4 form-field">
                                <label class="form-label">Link Pembelian <span
                                        class="text-[9px] text-slate-400 ml-1">(Opsional)</span></label>
                                <div class="input-group bg-white!">
                                    <div class="input-icon"><i class="fas fa-link text-emerald-400"></i></div>
                                    <input type="url" x-model="newPayment.link_pembelian"
                            ...
                            <div class="md:col-span-4 form-field">
                                ...
                                <div class="relative group/file">
                                    <input type="file" x-ref="paymentInput"
                                        class="absolute inset-0 opacity-0 cursor-pointer z-10" accept=".jpg,.jpeg,.png"
                                        @change="handlePaymentFile($event)" required>
                                    <div
                                        class="w-full px-4 h-12 flex items-center gap-3 rounded-xl border border-slate-200 bg-white group-hover/file:border-emerald-400 transition-all">
                                        <div
                                        ...
                                        <span
                                            class="text-xs font-semibold text-slate-400 group-hover/file:text-slate-600 truncate"
                                            x-text="newPayment.file ? newPayment.file.name : 'Pilih file nota (JPG/PNG)...'"></span>
                                    </div>
                                </div>
                            </div>
                        </div>
                        <div class="flex justify-center">
                            <button type="submit" :disabled="isLoadingPayment"
                                class="btn-accent h-12 px-10 rounded-2xl shadow-emerald-500/20">
            ...
            <?php if (empty($payments)): ?>
                <div class="flex flex-col items-center justify-center py-12 text-slate-300">
                    <div
                        class="w-16 h-16 rounded-full bg-slate-50 flex items-center justify-center mb-3 border border-slate-100">
                        <i class="fas fa-file-circle-exclamation text-2xl"></i>
                    </div>
                    <p class="text-xs font-bold uppercase tracking-widest text-slate-400">Belum Ada Bukti Pembayaran
                        Dilampirkan</p>
                </div>
            <?php endif; ?>
        </div>
    ...
    <div class="card-premium overflow-hidden animate-stagger delay-350" @mousemove="handleMouseMove">
        <div class="px-5 sm:px-7 py-4 border-b border-sky-50 bg-white/60 flex items-center justify-between">
            <div>
                ...
                <p class="text-[10px] text-slate-400 font-semibold mt-0.5 uppercase tracking-tighter">Upload dokumentasi
                    konsumsi saat sesi mentoring</p>
            </div>
            <span
                class="text-[10px] font-black bg-amber-100 text-amber-700 px-3 py-1 rounded-full uppercase tracking-widest"><?= count($konsumsi ?? []) ?>
                File</span>
        </div>
        <div class="p-5 sm:p-7">
            <!-- Konsumsi List Grid -->
            <div class="grid grid-cols-2 sm:grid-cols-3 md:grid-cols-4 lg:grid-cols-6 gap-4 mb-8">
                <?php foreach ($konsumsi ?? [] as $k): ?>
                    <div
                        class="group relative aspect-[3/4] rounded-2xl overflow-hidden border border-slate-200 bg-slate-50 hover:shadow-xl hover:shadow-amber-500/10 transition-all duration-300">
                        <img src="<?= base_url('mahasiswa/implementasi/konsumsi/' . $k->id) ?>"
                            alt="<?= esc($k->konsumsi_title) ?>" class="w-full h-full object-cover">

                        <div
                            class="absolute inset-0 bg-gradient-to-t from-amber-950/90 via-amber-900/20 to-transparent opacity-0 group-hover:opacity-100 transition-all duration-300 flex flex-col justify-end p-3">
                            <div class="flex items-center justify-center gap-2 mb-2">
                                <button
                                    @click="openLightbox('<?= base_url('mahasiswa/implementasi/konsumsi/' . $k->id) ?>', '<?= esc($k->konsumsi_title) ?>')"
                                    class="w-8 h-8 rounded-full bg-white/20 backdrop-blur-md text-white flex items-center justify-center hover:bg-white/40 transition-colors">
                                    <i class="fas fa-eye text-xs"></i>
                                </button>
                                <?php if ($canEdit): ?>
            ...
            <?php if ($canEdit): ?>
                ...
                <div
                    class="p-6 rounded-3xl bg-slate-50/50 border-2 border-dashed border-slate-200 hover:border-amber-300 hover:bg-amber-50/10 transition-all">
                        ...
                        <div class="grid md:grid-cols-2 gap-6">
                            <div class="form-field">
                                ...
                                <div class="input-group bg-white!">
                                    ...
                                    <input type="text" x-model="newKonsumsi.konsumsi_title"
                                        placeholder="Contoh: Makan Siang Mentoring 1" required>
                                </div>
                            </div>
                            <div class="form-field">
                                <label class="form-label">File Dokumentasi <span class="required">*</span></label>
                                <div class="relative group/file">
                                    <input type="file" x-ref="konsumsiInput"
                                        class="absolute inset-0 opacity-0 cursor-pointer z-10" accept=".jpg,.jpeg,.png"
                                        @change="handleKonsumsiFile($event)" required>
                                    <div
                                        class="w-full px-4 h-12 flex items-center gap-3 rounded-xl border border-slate-200 bg-white group-hover/file:border-amber-400 transition-all">
                                        <div
                                        ...
                                        <span
                                            class="text-xs font-semibold text-slate-400 group-hover/file:text-slate-600 truncate"
                                            x-text="newKonsumsi.file ? newKonsumsi.file.name : 'Pilih file dokumentasi (JPG/PNG)...'"></span>
                                    </div>
                                </div>
                            </div>
                        </div>
                        <div class="flex justify-center">
                            <button type="submit" :disabled="isLoadingKonsumsi"
                                class="btn-accent border-none! bg-amber-500! hover:bg-amber-600! h-12 px-10 rounded-2xl shadow-amber-500/20">
            ...
            <?php if (empty($konsumsi)): ?>
                <div class="flex flex-col items-center justify-center py-12 text-slate-300">
                    ...
                    <p class="text-xs font-bold uppercase tracking-widest text-slate-400">Belum Ada Bukti Konsumsi
                        Dilampirkan</p>
                </div>
            <?php endif; ?>
        </div>
    ...
    <template x-teleport="body">
        <div x-show="showLightbox" x-transition:enter="transition ease-out duration-300"
            ...
            x-transition:leave-end="opacity-0" class="fixed inset-0 z-[120]" :class="{ 'hidden': !showLightbox }"
            aria-labelledby="lightbox-modal-title" role="dialog" aria-modal="true">

            <!-- Backdrop -->
            <div class="fixed inset-0 bg-slate-900/50 backdrop-blur-sm transition-opacity" @click="closeLightbox()">
            </div>
                ...
                <div class="flex min-h-full items-center justify-center p-4 text-center sm:p-0">
                    <div
                        ...
                        <div class="px-6 py-5 bg-slate-50">
                            ...
                            <div class="mb-4">
                                <span
                                    class="inline-flex items-center gap-1.5 px-3 py-1 rounded-full text-xs font-bold border bg-emerald-50 text-emerald-600 border-emerald-200">
                                    <i class="fas fa-image text-[10px]"></i>
                                    <span class="truncate max-w-[300px]"
                                        x-text="lightboxTitle || 'Dokumentasi'">Dokumentasi</span>
                                </span>
                            </div>

                            <!-- Image Content -->
                            <div
                                class="rounded-xl overflow-hidden bg-white border border-slate-200 shadow-sm p-4 flex items-center justify-center min-h-[300px] max-h-[500px]">
                                <img :src="lightboxUrl" class="max-w-full max-h-[450px] rounded-lg object-contain"
                                    :alt="lightboxTitle">
                            </div>
                        </div>
    ...
    <!-- Edit Item Modal -->
    <template x-teleport="body">
        <div x-show="showEditItemModal" x-transition:enter="transition ease-out duration-300"
            x-transition:enter-start="opacity-0" x-transition:enter-end="opacity-100"
            x-transition:leave="transition ease-in duration-200" x-transition:leave-start="opacity-100"
            x-transition:leave-end="opacity-0" class="fixed inset-0 z-[100]" :class="{ 'hidden': !showEditItemModal }"
            aria-labelledby="edit-item-modal-title" role="dialog" aria-modal="true">

            <!-- Backdrop -->
            <div class="fixed inset-0 bg-slate-900/50 backdrop-blur-sm transition-opacity"
                @click="showEditItemModal = false"></div>
                    ...
                    <div
                        ...
                        <div class="bg-linear-to-r from-sky-500 to-sky-600 px-6 py-4">
                            <div class="flex items-center justify-between">
                                ...
                                <button type="button" @click="showEditItemModal = false"
                                    class="text-white/80 hover:text-white transition-colors">
                                    <i class="fas fa-times"></i>
                                </button>
                            </div>
                            <p class="text-[10px] text-white/80 font-black uppercase tracking-widest mt-1">Master Data
                                Implementasi</p>
                        </div>
                        ...
                        <form @submit.prevent="saveEditItem()" class="space-y-0">
                            <div class="px-6 py-5 space-y-5">
                                <div class="form-field">
                                    <label
                                        class="form-label text-[10px] font-black uppercase text-slate-400 tracking-widest">Nama
                                        Komponen</label>
                                    <div class="input-group group-focus-within:border-sky-500 transition-colors">
                                        <div class="input-icon"><i class="fas fa-tag"></i></div>
                                        <input type="text" x-model="editItem.item_title" placeholder="Nama komponen..."
                                            required>
                                    </div>
                                </div>
                                ...
                                <div class="form-field">
                                    <label
                                        class="form-label text-[10px] font-black uppercase text-slate-400 tracking-widest">Kategori/Justifikasi
                                        Pemakaian</label>
                                    <div class="input-group group-focus-within:border-sky-500 transition-colors">
                                        <div class="input-icon"><i class="fas fa-layer-group"></i></div>
                                        <select x-model="editItem.category"
                                            class="bg-transparent border-none outline-none w-full text-sm">
                                            <option value="">Pilih Kategori...</option>
                                            <option value="bahan">Bahan/Perlengkapan</option>
                                            <option value="alat">Alat/Mesin</option>
                                ...
                                <div class="grid grid-cols-2 gap-5">
                                    <div class="form-field">
                                        <label
                                            class="form-label text-[10px] font-black uppercase text-slate-400 tracking-widest">Kuantiti</label>
                                        <div class="input-group group-focus-within:border-sky-500 transition-colors">
                                            <div class="input-icon"><i class="fas fa-cubes"></i></div>
                                    ...
                                    <div class="form-field">
                                        <label
                                            class="form-label text-[10px] font-black uppercase text-slate-400 tracking-widest">Harga
                                            Satuan (Rp)</label>
                                        <div class="input-group group-focus-within:border-sky-500 transition-colors">
                                            <div class="input-icon text-emerald-500"><i class="fas fa-rupiah-sign"></i>
                                            </div>
                                ...
                                <div class="form-field">
                                    <label
                                        class="form-label text-[10px] font-black uppercase text-slate-400 tracking-widest">Deskripsi/Justifikasi
                                        Pemakaian</label>
                                    <div class="input-group p-0! group-focus-within:border-sky-500 transition-colors">
                                        <div class="input-icon pl-3"><i class="fas fa-align-left"></i></div>
                                        <textarea x-model="editItem.item_description" rows="3" class="px-0! py-3!"
                                            placeholder="Deskripsi penggunaan komponen..."></textarea>
                                    </div>
                                </div>
                            </div>
    ...
    <!-- Edit Payment Modal -->
    <template x-teleport="body">
        <div x-show="showEditPaymentModal" x-transition:enter="transition ease-out duration-300"
            x-transition:enter-start="opacity-0" x-transition:enter-end="opacity-100"
            x-transition:leave="transition ease-in duration-200" x-transition:leave-start="opacity-100"
            x-transition:leave-end="opacity-0" class="fixed inset-0 z-[100]"
            :class="{ 'hidden': !showEditPaymentModal }" aria-labelledby="edit-payment-modal-title" role="dialog"
            aria-modal="true">

            <!-- Backdrop -->
            <div class="fixed inset-0 bg-slate-900/50 backdrop-blur-sm transition-opacity"
                @click="showEditPaymentModal = false"></div>
                    ...
                    <div
                        ...
                        <div class="bg-linear-to-r from-emerald-500 to-teal-600 px-6 py-4">
                            <div class="flex items-center justify-between">
                                ...
                                <button type="button" @click="showEditPaymentModal = false"
                                    class="text-white/80 hover:text-white transition-colors">
                                    <i class="fas fa-times"></i>
                                </button>
                            </div>
                            <p class="text-[10px] text-white/80 font-black uppercase tracking-widest mt-1">Update Bukti
                                Transaksi</p>
                        </div>
                        ...
                        <form @submit.prevent="saveEditPayment()" class="space-y-0">
                            <div class="px-6 py-5">
                                <div class="form-field">
                                    <label
                                        class="form-label text-[10px] font-black uppercase text-slate-400 tracking-widest">Keterangan
                                        / Nama Nota</label>
                                    <div class="input-group group-focus-within:border-emerald-500 transition-colors">
                                        ...
                                        <input type="text" x-model="editPayment.payment_title"
                                            placeholder="Contoh: Nota Pembelian Alat..." required>
                                    </div>
                                </div>
                                <div class="form-field mt-4">
                                    <label
                                        class="form-label text-[10px] font-black uppercase text-slate-400 tracking-widest">Link
                                        Pembelian <span class="text-slate-300 ml-1">(Opsional)</span></label>
                                    <div class="input-group group-focus-within:border-emerald-500 transition-colors">
                                        <div class="input-icon"><i class="fas fa-link text-emerald-400"></i></div>
                                        <input type="url" x-model="editPayment.link_pembelian"
                                            placeholder="https://...">
                                    </div>
                                </div>
                            </div>
                            ...
                            <div class="bg-slate-50 px-6 py-4 flex gap-3 justify-end border-t border-slate-100">
                                ...
                                <button type="submit" :disabled="isLoadingEditPayment"
                                    class="btn-primary text-sm shadow-lg shadow-emerald-500/20"
                                    style="background-color: #10b981; border: none;">
                                    <i class="fas fa-check-circle mr-2"
                                        :class="isLoadingEditPayment ? 'fa-spin fa-spinner' : ''"></i>
    ...
    <!-- Edit Konsumsi Modal -->
    <template x-teleport="body">
        <div x-show="showEditKonsumsiModal" x-transition:enter="transition ease-out duration-300"
            x-transition:enter-start="opacity-0" x-transition:enter-end="opacity-100"
            x-transition:leave="transition ease-in duration-200" x-transition:leave-start="opacity-100"
            x-transition:leave-end="opacity-0" class="fixed inset-0 z-[100]"
            :class="{ 'hidden': !showEditKonsumsiModal }" aria-labelledby="edit-konsumsi-modal-title" role="dialog"
            aria-modal="true">

            <!-- Backdrop -->
            <div class="fixed inset-0 bg-slate-900/50 backdrop-blur-sm transition-opacity"
                @click="showEditKonsumsiModal = false"></div>
                    ...
                    <div
                        ...
                        <div class="bg-linear-to-r from-amber-500 to-orange-600 px-6 py-4">
                            <div class="flex items-center justify-between">
                                ...
                                <button type="button" @click="showEditKonsumsiModal = false"
                                    class="text-white/80 hover:text-white transition-colors">
                                    <i class="fas fa-times"></i>
                                </button>
                            </div>
                            <p class="text-[10px] text-white/80 font-black uppercase tracking-widest mt-1">Update
                                Dokumentasi</p>
                        </div>
                        ...
                        <form @submit.prevent="saveEditKonsumsi()" class="space-y-0">
                            <div class="px-6 py-5">
                                <div class="form-field">
                                    ...
                                    <div class="input-group group-focus-within:border-amber-500 transition-colors">
                                        ...
                                        <input type="text" x-model="editKonsumsi.konsumsi_title"
                                            placeholder="Contoh: Makan Siang Rapat..." required>
                                    </div>
                                </div>
                            </div>
                            ...
                            <div class="bg-slate-50 px-6 py-4 flex gap-3 justify-end border-t border-slate-100">
                                ...
                                <button type="submit" :disabled="isLoadingEditKonsumsi"
                                    class="btn-primary text-sm shadow-lg shadow-amber-500/20"
                                    style="background-color: #f59e0b; border: none;">
                                    <i class="fas fa-check-circle mr-2"
                                        :class="isLoadingEditKonsumsi ? 'fa-spin fa-spinner' : ''"></i>
