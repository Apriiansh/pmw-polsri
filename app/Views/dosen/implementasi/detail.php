...
<div class="space-y-6" x-data="{
    ...
    <div class="flex flex-col sm:flex-row sm:items-center sm:justify-between gap-4 animate-stagger">
        <div>
            ...
            <p class="section-subtitle text-[10px] sm:text-[11px]">Penilaian realisasi barang dan dokumentasi oleh Dosen Pendamping</p>
        </div>
        <a href="<?= base_url('dosen/implementasi') ?>" class="btn-ghost inline-flex items-center gap-2 text-slate-500 hover:text-sky-600 transition-colors">
    ...
    <?php if (!$proposal): ?>
        <div class="card-premium p-12 text-center animate-stagger">
            <div class="w-20 h-20 rounded-3xl bg-rose-50 text-rose-500 flex items-center justify-center mx-auto mb-4 shadow-xl shadow-rose-100/50">
                <i class="fas fa-exclamation-triangle text-3xl"></i>
            </div>
            <h3 class="text-lg font-black text-slate-800">Proposal Tidak Ditemukan</h3>
            <p class="text-xs text-slate-400 mt-2">Data proposal mungkin telah dihapus atau Anda tidak memiliki akses.</p>
        </div>
    <?php else: ?>
        ...
        $currentStatus = $selection->dosen_status ?: 'pending';
    ...
    ================================================================= -->
        <div class="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-6 animate-stagger">
            ...
            <!-- CARD: HERO BUSINESS & PROGRESS (Col 3) -->
            ...
            <div class="lg:col-span-1 card-premium flex flex-col justify-between overflow-hidden group/status" @mousemove="handleMouseMove">
                <div class="p-6 flex flex-col h-full">
                    <p class="text-[10px] font-black uppercase tracking-widest text-slate-400 mb-6">Status Anda</p>
                    ...
                    <div class="flex-1 flex flex-col items-center justify-center text-center">
                        <div class="inline-flex items-center justify-center w-20 h-20 rounded-3xl bg-slate-50 border-4 border-white shadow-xl mb-4 transition-transform group-hover/status:scale-110">
                            ...
                            <?php elseif ($currentStatus === 'submitted'): ?>
                                <i class="fas fa-paper-plane text-4xl text-sky-500"></i>
                            <?php else: ?>
                                <i class="fas fa-hourglass-half text-4xl text-slate-300 opacity-20"></i>
                            <?php endif; ?>
                        </div>
                    ...
                    <div class="mt-6 pt-4 border-t border-slate-50">
                        <div class="flex items-center justify-between text-[10px]">
                            <span class="font-bold text-slate-400 uppercase tracking-tighter">Dikirim MHS Pada</span>
                            <span class="font-black text-slate-700"><?= !empty($selection->student_submitted_at) ? date('d M, H:i', strtotime($selection->student_submitted_at)) : '-' ?></span>
                        </div>
                    </div>
                </div>
            </div>
            ...
            <div class="lg:col-span-2 card-premium overflow-hidden" @mousemove="handleMouseMove">
                <div class="p-6">
                    <div class="flex items-center justify-between mb-6">
                        <h4 class="text-xs font-black uppercase tracking-widest text-slate-400">Anggota Tim Mahasiswa</h4>
                        ...
                        <?php foreach ($members as $member): ?>
                            <div class="flex items-center gap-3 p-3 rounded-2xl bg-white border border-slate-50 hover:border-sky-200 hover:shadow-md transition-all cursor-pointer group"
                                ...
                                <div class="min-w-0">
                                    ...
                                    <p class="text-[9px] font-black text-slate-400 uppercase tracking-tighter mt-0.5"><?= esc($member['role'] === 'ketua' ? 'Ketua' : 'Anggota') ?></p>
                                </div>
                            </div>
                        <?php endforeach; ?>
                    </div>
            ...
            <!-- CARD: ADMIN FINAL STATUS (Col 2) -->
            <div class="lg:col-span-2 card-premium overflow-hidden group/admin" @mousemove="handleMouseMove">
                <div class="p-6">
                    <h4 class="text-xs font-black uppercase tracking-widest text-slate-400 mb-6">Status Akhir Admin</h4>

                    <div class="p-4 rounded-2xl bg-slate-50 border border-slate-100 flex flex-col sm:flex-row items-center gap-6">
                        <div class="shrink-0">
                            <div class="w-14 h-14 rounded-2xl bg-white border border-slate-100 text-violet-500 flex items-center justify-center shadow-sm">
                                <i class="fas fa-shield-halved text-2xl transition-transform group-hover/admin:rotate-12"></i>
                            </div>
                        </div>
                        ...
                        <?php elseif ($selection->admin_status === 'revision'): ?>
                            <div class="shrink-0 flex items-center gap-2 px-3 py-1.5 rounded-xl bg-orange-500 text-white shadow-lg shadow-orange-100">
                                <i class="fas fa-rotate-left text-xs"></i>
                                <span class="text-[10px] font-black uppercase tracking-widest">REVISION</span>
                            </div>
                        <?php else: ?>
                            <div class="shrink-0 flex items-center gap-2 px-3 py-1.5 rounded-xl bg-slate-200 text-slate-500">
                    ...
                    <?php if ($selection->admin_catatan): ?>
                        <div class="mt-4 p-4 rounded-xl bg-white border border-slate-50 italic text-[11px] text-slate-500 leading-relaxed relative">
                            ...
                            <span class="relative z-10">&quot;<?= esc($selection->admin_catatan) ?>&quot;</span>
                        </div>
                    <?php endif; ?>
                </div>
            </div>
                                        ...
                                        <div class="flex flex-wrap sm:flex-nowrap gap-2">
                                            <?php if (!empty($item->photos)): ?>
                                                <?php foreach ($item->photos as $ph): ?>
                                                    <button type="button"
                                                        onclick="openImagePreview('<?= base_url('dosen/implementasi/photo/' . $ph->id) ?>', 'Foto Komponen: <?= esc($item->item_title) ?>')"
                                                        ...
                                                        <img src="<?= base_url('dosen/implementasi/photo/' . $ph->id) ?>" alt="Photo" class="w-full h-full object-cover group-hover:scale-110 transition-transform">
                ...
                <div class="card-premium flex-1 overflow-hidden" @mousemove="handleMouseMove">
                    ...
                    <div class="p-4 flex items-center justify-start overflow-x-auto no-scrollbar py-8">
                        <?php if (empty($konsumsi)): ?>
                            <div class="w-full py-6 text-center text-slate-300 text-[10px] font-bold uppercase tracking-widest">Kosong</div>
                        <?php else: ?>
                            <div class="flex items-center -space-x-4 px-2">
                                <?php foreach ($konsumsi as $idx => $k): ?>
                                    <button type="button"
                                        onclick="openImagePreview('<?= base_url('dosen/implementasi/konsumsi/' . $k->id) ?>', 'Dokumentasi Konsumsi #<?= $idx + 1 ?>')"
                                        ...
                                        <img src="<?= base_url('dosen/implementasi/konsumsi/' . $k->id) ?>" alt="Konsumsi" class="w-full h-full object-cover">
                                        <div class="absolute inset-0 bg-black/20 opacity-0 group-hover:opacity-100 flex items-center justify-center transition-opacity">
                                            <i class="fas fa-expand text-white text-[10px]"></i>
                                        </div>
                                    </button>
            ...
            <!-- CARD: DOSEN VALIDATION FORM (Col 4) -->
            <div class="lg:col-span-4">
                <div class="card-premium overflow-hidden animate-stagger delay-400 border-l-4 border-l-sky-500" @mousemove="handleMouseMove" id="validation-section">
                    <div class="px-5 sm:px-7 py-4 sm:py-5 border-b border-sky-50 bg-white/60">
                        ...
                        <p class="text-[11px] text-(--text-muted) mt-0.5">Berikan penilaian dan feedback untuk laporan implementasi mahasiswa</p>
                    </div>

                    <form action="<?= base_url('dosen/implementasi/' . $proposal['id'] . '/validate') ?>" method="post" id="validationForm">
                        <?= csrf_field() ?>
                        <div class="p-5 sm:p-7 space-y-6">
                                    ...
                                    <label class="relative cursor-pointer">
                                        ...
                                        <div class="p-4 rounded-2xl border-2 border-slate-100 peer-checked:border-emerald-500 peer-checked:bg-emerald-50 transition-all hover:border-emerald-200 group">
                                            <div class="flex items-center gap-3">
                                                ...
                                                <div>
                                                    <p class="font-bold text-(--text-heading) text-sm leading-tight">Setujui</p>
                                                    <p class="text-[9px] text-slate-400 font-bold uppercase tracking-tighter">Laporan Sah</p>
                                                </div>
                                            </div>
                                        </div>
                                    </label>
                            ...
                            <div class="space-y-1.5">
                                ...
                                <div class="input-group items-start py-2 focus-within:ring-4 focus-within:ring-sky-50 transition-all">
                                    ...
                                    <textarea name="catatan" rows="4" placeholder="Berikan feedback membangun untuk mahasiswa..." class="bg-transparent border-none outline-none w-full text-sm font-medium leading-relaxed"><?= esc($selection->dosen_catatan ?? '') ?></textarea>
                                </div>
                            </div>
...
<script>
    function openBiodataModal(type, data) {
        ...
        roleBadge.className = `inline-flex items-center gap-1.5 px-4 py-1.5 rounded-full text-[10px] font-black border bg-teal-50 text-teal-600 border-teal-200 uppercase tracking-widest`;
    ...
    function openImagePreview(url, title) {
        ...
        // Set title
        titleSpan.textContent = title || 'Dokumentasi';
            ...
            <div class="relative transform overflow-hidden rounded-2xl bg-white text-left shadow-xl transition-all sm:my-8 sm:w-full sm:max-w-4xl">
                ...
                <div class="px-6 py-5 bg-slate-50">
                    ...
                    <div class="mb-4">
                        <span id="previewTitle" class="inline-flex items-center gap-1.5 px-3 py-1 rounded-full text-xs font-bold border bg-emerald-50 text-emerald-600 border-emerald-200">
                            ...
                            <span class="truncate max-w-[300px]">Dokumentasi</span>
                        </span>
                    </div>
                    ...
                    <div class="rounded-xl overflow-hidden bg-white border border-slate-200 shadow-sm p-4 flex items-center justify-center min-h-[300px] max-h-[500px]">
                        <img id="previewImg" src="" class="max-w-full max-h-[450px] rounded-lg object-contain">
                    </div>
                </div>
                ...
                <div class="bg-white px-6 py-4 flex justify-between items-center border-t border-slate-100">
                    <div class="flex items-center gap-2 text-xs text-slate-400">
                        ...
                        <span id="previewFileInfo">Image Preview</span>
                    </div>
