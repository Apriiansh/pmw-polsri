...
<div class="space-y-8" x-data="{
    ...
    <!-- Teams Info -->
    <div class="card-premium p-6 animate-stagger delay-500" @mousemove="handleMouseMove">
        <h3 class="font-display text-base font-bold text-(--text-heading) mb-4 border-b border-sky-50 pb-3">
            <i class="fas fa-users-viewfinder mr-2 text-sky-500"></i>Tim Bimbingan Anda
        </h3>
        ...
        <div class="card-premium w-full max-w-3xl bg-white shadow-2xl animate-modal max-h-[90vh] overflow-hidden" @click.away="showVerifyModal = false">
            ...
            <div class="overflow-y-auto p-6 space-y-6 max-h-[60vh]" x-if="selectedLogbook">
                ...
                <div class="flex gap-3">
                    <a :href="`<?= base_url('dosen/kegiatan/file/photo') ?>/${selectedLogbook?.id}`" target="_blank" 
                    ...
                    <a :href="`<?= base_url('dosen/kegiatan/file/supervisor') ?>/${selectedLogbook?.id}`" target="_blank"
            ...
            <!-- Verification Form -->
            <form :action="`<?= base_url('dosen/kegiatan/verify') ?>/${selectedLogbook?.id}`" method="POST" class="p-6 border-t border-slate-100 bg-slate-50/50">
                <?= csrf_field() ?>
                <div class="space-y-4">
