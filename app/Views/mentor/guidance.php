    ...
    <div x-show="showScheduleModal" 
        ...
        <div class="card-premium w-full max-w-lg bg-white shadow-2xl animate-modal" @click.away="showScheduleModal = false">
            ...
            <form action="<?= base_url('mentor/mentoring/schedule') ?>" method="POST" class="p-6 space-y-6">
                ...
                <div class="form-field">
                    <label class="form-label">Tanggal &amp; Waktu Selesai <span class="required">*</span></label>
                    <div class="grid grid-cols-2 gap-5">
                        <div class="form-field">
                    ...
                    <p class="text-[10px] text-slate-400 mt-2 ml-1 italic">Waktu selesai pengisian logbook oleh mahasiswa sampai <span class="font-bold text-amber-600">tanggal &amp; jam</span> tersebut.</p>
                </div>
        ...
        <div class="card-premium w-full max-w-3xl bg-white shadow-2xl animate-modal overflow-hidden max-h-[90vh] flex flex-col" @click.away="showVerifyModal = false">
            ...
            <div class="overflow-y-auto p-6 space-y-8 flex-1 custom-scrollbar" x-show="selectedLogbook">
                    ...
                    <div class="space-y-6">
                        <div class="space-y-1.5">
                            ...
                            <div class="aspect-video rounded-2xl overflow-hidden border-2 border-slate-100 bg-slate-50 group relative">
                                <img :src="selectedLogbook?.id ? `<?= base_url('mentor/mentoring/file/photo') ?>/${selectedLogbook.id}` : ''" class="w-full h-full object-cover">
                                <a :href="selectedLogbook?.id ? `<?= base_url('mentor/mentoring/file/photo') ?>/${selectedLogbook.id}` : '#'" target="_blank" 
                        ...
                        <div class="space-y-1.5" x-show="selectedLogbook?.assignment_file">
                            ...
                            <a :href="selectedLogbook?.id ? `<?= base_url('mentor/mentoring/file/assignment') ?>/${selectedLogbook.id}` : '#'" target="_blank" class="flex items-center gap-3 p-3 rounded-xl border border-amber-100 bg-amber-50 text-amber-600 hover:bg-amber-100 transition-all group w-full">
                ...
                <div class="pt-6 border-t border-slate-100">
                    ...
                    <template x-if="selectedLogbook && selectedLogbook.status !== 'approved'">
                        <form :action="selectedLogbook ? `<?= base_url('mentor/mentoring/verify') ?>/${selectedLogbook.id}` : '#'" method="POST">
                            <?= csrf_field() ?>
                            <div class="space-y-6">
