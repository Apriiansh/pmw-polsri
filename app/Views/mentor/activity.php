...
<div class="space-y-8" x-data="{
    ...
    <div class="grid grid-cols-1 lg:grid-cols-3 gap-8">
                        ...
                        <tbody>
                            ...
                            <?php else: ?>
                                ...
                                <tr class="group">
                                    ...
                                    <td>
                                        <div class="text-[11px] text-slate-500 italic max-w-[150px] truncate" title="<?= esc($logbook->mentor_note) ?>">
        ...
        <!-- Sidebar: Teams Info -->
        <div class="lg:col-span-1 space-y-6 animate-stagger delay-400">
    ...
    <div x-show="showVerifyModal" 
        ...
        <div class="card-premium w-full max-w-3xl bg-white shadow-2xl animate-modal max-h-[90vh] overflow-hidden" @click.away="showVerifyModal = false">
            ...
            <div class="overflow-y-auto p-4 space-y-6 max-h-[60vh]" x-if="selectedLogbook">
                ...
                <div class="grid grid-cols-1 md:grid-cols-2 gap-6" x-show="selectedLogbook?.photo_supervisor_visit || selectedLogbook?.video_url">
                    ...
                    <div class="space-y-2" x-show="selectedLogbook?.photo_supervisor_visit">
                        <label class="text-[10px] font-black text-slate-400 uppercase tracking-widest">Preview Foto Kunjungan</label>
                ...
                <div class="space-y-2" x-show="selectedLogbook?.gallery && selectedLogbook.gallery.length > 0">
                    ...
                    <div class="grid grid-cols-2 sm:grid-cols-4 gap-2">
                        <template x-for="photo in selectedLogbook?.gallery" :key="photo.id">
                            <a :href="`<?= base_url('mentor/kegiatan/gallery') ?>/${photo.id}`" target="_blank" class="aspect-square rounded-lg overflow-hidden border border-slate-100 bg-slate-50 group relative">
                                <img :src="`<?= base_url('mentor/kegiatan/gallery') ?>/${photo.id}`" class="w-full h-full object-cover">
            ...
            <!-- Verification Form -->
            <form :action="`<?= base_url('mentor/kegiatan/verify') ?>/${selectedLogbook?.id}`" method="POST" 
                  class="p-6 border-t border-slate-100 bg-slate-50/50"
                  x-show="['approved_by_dosen'].includes(selectedLogbook?.status)">
                <?= csrf_field() ?>
                <div class="space-y-4">
            ...
            <div class="p-6 border-t border-slate-100 bg-slate-50/50 flex justify-end" x-show="!['approved_by_dosen'].includes(selectedLogbook?.status)">
    ...
    <div x-show="showTeamModal" 
        ...
        <div class="card-premium w-full max-w-2xl bg-white shadow-2xl animate-modal flex flex-col max-h-[90vh] overflow-hidden" @click.away="showTeamModal = false">
            ...
            <div class="overflow-y-auto p-6 space-y-6 flex-1 custom-scrollbar">
                ...
                <!-- Mentor Notes (History Only) -->
                <div class="space-y-2 p-4 mt-4 rounded-xl bg-indigo-50 border border-indigo-100" x-show="!['approved_by_dosen'].includes(selectedLogbook?.status) && selectedLogbook?.mentor_note">
                    <label class="text-[10px] font-black text-indigo-400 uppercase tracking-widest">Catatan Anda Sebelumnya</label>
                    <p class="text-[13px] text-indigo-700 italic" x-text="`&quot;${selectedLogbook?.mentor_note}&quot;`"></p>
                </div>
            </div>
